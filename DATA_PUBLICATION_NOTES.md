...
- 公開CSVから、調査メモ、作業ステータス、レビュー用集計、座標取得元URL、座標レビュー列を除外した。
...
- 学校地点の座標は第三者地図サイト由来のものを再配布せず、地図表示と距離計算には GSI 住所検索で取得した住所代表点を使う構成にした。
- フィルター用の学科カテゴリを、追加された専門学科に合わせて6分類へ再編した。
...
- GSI 住所検索で学校住所の代表点を取得し、公開CSVには座標取得元URLや内部レビュー列を含めず、座標値と出典ラベルだけを残す。
- HeartRails Geo API は商用・非商用を問わず無料利用可能とされ、アプリ内に指定クレジットを表示する条件があるため、基準郵便番号からの緯度経度取得に使う。
